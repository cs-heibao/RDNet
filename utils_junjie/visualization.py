...
def draw_xywha(im, x, y, w, h, color=(255,0,0), linewidth=5):
    '''
    ...
    rot_pts = []
    for pt in pts:
        rot_pts.append(([x, y] + pt).astype(int))
...
def draw_dt_on_np(im, detections, print_dt=False, color=(255,0,0),
    ...
    for bb in detections:
        ...
        x1, y1 = x - w/2, y - h/2
        if print_dt:
            print('[{} {} {} {}], confidence: {}'.format(x, y, w, h, conf))
        draw_xywha(im, x, y, w, h, color=color, linewidth=line_width)
        if kwargs.get('show_conf', True):
            cv2.putText(im, '{}'.format('%.2f'%conf), (int(x1),int(y1)), font, 1*text_size,
